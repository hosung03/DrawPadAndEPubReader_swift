...
public enum EPubReaderFont: Int {
	case andada = 0
	case lato
	case lora
	case raleway
  
	public static func EPubReaderFont(fontName: String) -> EPubReaderFont? {
		var font: EPubReaderFont?
		switch fontName {
...
public enum EPubReaderFontSize: Int {
	case xs = 0
	case s
	case m
	case l
	...
	public static func EPubReaderFontSize(fontSizeStringRepresentation: String) -> EPubReaderFontSize? {
		var fontSize: EPubReaderFontSize?
...
class EPubReaderFontsMenu: UIViewController, SMSegmentViewDelegate, UIGestureRecognizerDelegate {
    ...
    override func viewDidLoad() {
        ...
        // Tap gesture
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(EPubReaderFontsMenu.tapGesture))
        ...
        dayNight.selectSegmentAtIndex(EPubReader.nightMode.hashValue)
		...
		fontName.selectSegmentAtIndex(EPubReader.currentFont.rawValue)
        ...
        slider.value = CGFloat(EPubReader.currentFontSize.rawValue)
        slider.addTarget(self, action: #selector(EPubReaderFontsMenu.sliderValueChanged(_:)), for: UIControlEvents.valueChanged)
        ...
        var scrollDirection = EPubReaderScrollDirection(rawValue: EPubReader.currentScrollDirection)
    ...
    func segmentView(_ segmentView: SMSegmentView, didSelectSegmentAtIndex index: Int) {
        guard (EPubReader.shared.readerCenter?.currentPage) != nil else { return }
        
        if segmentView.tag == 1 {

			EPubReader.nightMode = Bool(index == 1)

			UIView.animate(withDuration: 0.6, animations: {
				self.menuView.backgroundColor = (EPubReader.nightMode ?readerConfig.nightModeBackground : UIColor.white)
			})

		} else if segmentView.tag == 2 {

			EPubReader.currentFont = EPubReaderFont(rawValue: index)!

        }  else if segmentView.tag == 3 {

			guard EPubReader.currentScrollDirection != index else { return }
            
            EPubReader.currentScrollDirection = index
        }
    }
    ...
    func sliderValueChanged(_ sender: HADiscreteSlider) {
		...
		if let _fontSize = EPubReaderFontSize(rawValue: index) {
			EPubReader.currentFontSize = _fontSize
		}
    }
    ...
    func tapGesture() {
        ...
        if readerConfig.shouldHideNavigationOnTap == false {
            EPubReader.shared.readerCenter?.showBars()
        }
    }
