...
internal var kCurrentScrollDirection = "kCurrentScrollDirection"
...
/// EPubReader actions delegate
@objc public protocol EPubReaderDelegate: class {
    
    /**
     Did finished loading book.
     
     - parameter EPubReader: The EPubReader instance
     - parameter book:        The Book instance
     */
    @objc optional func EPubReader(_ EPubReader: EPubReader, didFinishedLoading book: FRBook)
    
    /**
     Called when reader did closed.
     */
    @objc optional func EPubReaderDidClosed()
}
...
open class EPubReader: NSObject {
    ...
    /// Singleton instance
    open static let shared = EPubReader()
    ...
    /// EPubReaderDelegate
    open weak var delegate: EPubReaderDelegate?
    
    
    open weak var readerCenter: EPubReaderCenter?
    open weak var readerContainer: EPubReaderContainer!
    //open weak var readerAudioPlayer: EPubReaderAudioPlayer?
    ...
    open static var nightMode: Bool {
        get { return EPubReader.defaults.bool(forKey: kNightMode) }
        set (value) {
            EPubReader.defaults.set(value, forKey: kNightMode)
			EPubReader.defaults.synchronize()

			if let readerCenter = EPubReader.shared.readerCenter {
    ...
    open static var currentFont: EPubReaderFont {
		get { return EPubReaderFont(rawValue: EPubReader.defaults.value(forKey: kCurrentFontFamily) as! Int)! }
        set (font) {
            EPubReader.defaults.setValue(font.rawValue, forKey: kCurrentFontFamily)
			_ = EPubReader.shared.readerCenter?.currentPage?.webView.js("setFontName('\(font.cssIdentifier)')")
        }
    }
    
    /// Check current font size
    open static var currentFontSize: EPubReaderFontSize {
		get { return EPubReaderFontSize(rawValue: EPubReader.defaults.value(forKey: kCurrentFontSize) as! Int)! }
        set (value) {
            EPubReader.defaults.setValue(value.rawValue, forKey: kCurrentFontSize)

			if let _currentPage = EPubReader.shared.readerCenter?.currentPage {
    ...
    static var currentAudioRate: Int {
        get { return EPubReader.defaults.value(forKey: kCurrentAudioRate) as! Int }
        set (value) {
            EPubReader.defaults.setValue(value, forKey: kCurrentAudioRate)
        }
    }

    /// Check the current highlight style
    static var currentHighlightStyle: Int {
        get { return EPubReader.defaults.value(forKey: kCurrentHighlightStyle) as! Int }
        set (value) {
            EPubReader.defaults.setValue(value, forKey: kCurrentHighlightStyle)
        }
    }
    
    /// Check the current Media Overlay or TTS style
    static var currentMediaOverlayStyle: MediaOverlayStyle {
        get { return MediaOverlayStyle(rawValue: EPubReader.defaults.value(forKey: kCurrentMediaOverlayStyle) as! Int)! }
        set (value) {
            EPubReader.defaults.setValue(value.rawValue, forKey: kCurrentMediaOverlayStyle)
        }
    }
    
    /// Check the current scroll direction
    open static var currentScrollDirection: Int {
        get { return EPubReader.defaults.value(forKey: kCurrentScrollDirection) as! Int }
        set (value) {
            EPubReader.defaults.setValue(value, forKey: kCurrentScrollDirection)

			if let _readerCenter = EPubReader.shared.readerCenter  {
				let direction = EPubReaderScrollDirection(rawValue: currentScrollDirection) ?? .defaultVertical
    ...
    // MARK: - Present EPub Reader
    
    /**
     Present a EPub Reader for a Parent View Controller.
     */
    open class func presentReader(parentViewController: UIViewController, withEpubPath epubPath: String, andConfig config: EPubReaderConfig, shouldRemoveEpub: Bool = true, animated: Bool = true) {
        let reader = EPubReaderContainer(withConfig: config, epubPath: epubPath, removeEpub: shouldRemoveEpub)
        EPubReader.shared.readerContainer = reader
    ...
    open class func saveReaderState() {
        guard EPubReader.isReaderOpen else { return }
        
        if let currentPage = EPubReader.shared.readerCenter?.currentPage {
            let position = [
            ...
            EPubReader.defaults.set(position, forKey: kBookId)
        }
    }
    ...
    open class func close() {
        ...
        EPubReader.isReaderOpen = false
        EPubReader.isReaderReady = false
        //EPubReader.shared.readerAudioPlayer?.stop(immediate: true)
        EPubReader.defaults.set(0, forKey: kCurrentTOCMenu)
        EPubReader.shared.delegate?.EPubReaderDidClosed?()
    }
}
...
func isNight<T> (_ f: T, _ l: T) -> T {
    return EPubReader.nightMode ? f : l
}
...
internal extension Bundle {
    class func frameworkBundle() -> Bundle {
        return Bundle(for: EPubReader.self)
    }
}
