...
/// Protocol which is used from `EPubReaderCenter`s.
@objc public protocol EPubReaderCenterDelegate: class {
	 ...
	 */
	@objc optional func pageDidAppear(_ page: EPubReaderPage)

	/**
     Passes and returns the HTML content as `String`. Implement this method if you want to modify the HTML content of a `EPubReaderPage`.

     - parameter page: The `EPubReaderPage`
     - parameter htmlContent: The current HTML content as `String`

     - returns: The adjusted HTML content as `String`. This is the content which will be loaded into the given `EPubReaderPage`
	 */
	@objc optional func htmlContentForPage(_ page: EPubReaderPage, htmlContent: String) -> String
}

/// The base reader class
open class EPubReaderCenter: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource {

	/// This delegate receives the events from the current `EPubReaderPage`s delegate.
	open weak var delegate: EPubReaderCenterDelegate?

	/// This delegate receives the events from current page
	open weak var pageDelegate: EPubReaderPageDelegate?

    
    /// The current visible page on reader
    open fileprivate(set) var currentPage: EPubReaderPage?
    ...
    var pageIndicatorView: EPubReaderPageIndicator?
    ...
    override open func viewDidLoad() {
        ...
        collectionView!.register(EPubReaderPage.self, forCellWithReuseIdentifier: reuseIdentifier)
        ...
        if !readerConfig.hidePageIndicator {
            pageIndicatorView = EPubReaderPageIndicator(frame: self.frameForPageIndicatorView())
	...
	/**
     Enable or disable the scrolling between chapters (`EPubReaderPage`s). If this is enabled it's only possible to read the current chapter. If another chapter should be displayed is has to be triggered programmatically with `changePageWith`.

     - parameter scrollEnabled: `Bool` which enables or disables the scrolling between `EPubReaderPage`s.
	 */
    ...
    func configureNavBarButtons() {
        ...
        navigationItem.leftBarButtonItem = UIBarButtonItem(image: closeIcon, style: .plain, target: self, action:#selector(closeReader(_:)))
        
        let actionIcon = UIImage(readerImageNamed: "icon-navbar-share")?.ignoreSystemTint()
        navigationItem.rightBarButtonItem = UIBarButtonItem(image: actionIcon, style: .plain, target: self, action: #selector(viewActionMenu(_:)))
    ...
    func reloadData() {
        ...
        if let position = EPubReader.defaults.value(forKey: kBookId) as? NSDictionary,
    ...
    func setPageProgressiveDirection(_ page: EPubReaderPage) {
        if EPubReader.needsRTLChange {
//            if page.transform.a == -1 { return }
    ...
    func setScrollDirection(_ direction: EPubReaderScrollDirection) {
    ...
    func hideBars() {
        ...
        let shouldHide = true
        EPubReader.shared.readerContainer.shouldHideStatusBar = shouldHide
        
        UIView.animate(withDuration: 0.25, animations: {
            EPubReader.shared.readerContainer.setNeedsStatusBarAppearanceUpdate()
...
//            self.pageIndicatorView.minutesLabel.alpha = shouldHide ? 0 : 1
    ...
    open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: reuseIdentifier, for: indexPath) as! EPubReaderPage
		...
		if let resource = book.spine.spineReferences[(indexPath as NSIndexPath).row].resource,
			...
			// Font class name
			var classes = EPubReader.currentFont.cssIdentifier
			classes += " "+EPubReader.currentMediaOverlayStyle.className()

			// Night mode
			if EPubReader.nightMode {
				classes += " nightMode"
			}

			// Font Size
			classes += " \(EPubReader.currentFontSize.cssIdentifier)"
    ...
    override open func willRotate(to toInterfaceOrientation: UIInterfaceOrientation, duration: TimeInterval) {
        guard EPubReader.isReaderReady else { return }
    ...
    func updateCurrentPage(_ page: EPubReaderPage? = nil, completion: (() -> Void)? = nil) {
        if let page = page {
            currentPage = page
			...
			currentPage = collectionView.cellForItem(at: currentIndexPath) as? EPubReaderPage
...
//        } else { title = ""}
    ...
    func pagesForCurrentPage(_ page: EPubReaderPage?) {
    ...
    */
    ...
    func shareChapter(_ sender: UIBarButtonItem) {
        ...
        if let chapterText = currentPage.webView.js("getBodyText()") {
            ...
            let act = EPubReaderSharingProvider(subject: subject, text: text, html: html)
        ...
        if scrollView is UICollectionView {} else {
            ...
            if let page = currentPage
				...
				if (readerConfig.scrollDirection == .horizontalWithVerticalContent),
					let cell = ((scrollView.superview as? UIWebView)?.delegate as? EPubReaderPage) {
    ...
    open func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        recentlyScrolledTimer = Timer(timeInterval:recentlyScrolledDelay, target: self, selector: #selector(EPubReaderCenter.clearRecentlyScrolled), userInfo: nil, repeats: false)
...
//        EPubReader.saveReaderState()
//        
//        let chapter = EPubReaderChapterList()
//        chapter.delegate = self
//        let highlight = EPubReaderHighlightList()
//        
...
// MARK: EPubPageDelegate

extension EPubReaderCenter: EPubReaderPageDelegate {
    
    public func pageDidLoad(_ page: EPubReaderPage) {
        ...
        if let position = EPubReader.defaults.value(forKey: kBookId) as? NSDictionary {
            ...
            } else if !isScrolling && EPubReader.needsRTLChange {
	...
	public func pageWillLoad(_ page: EPubReaderPage) {
		// Pass the event to the centers `pageDelegate`
...
extension EPubReaderCenter: EPubReaderChapterListDelegate {
    ...
    func chapterList(didDismissedChapterList chapterList: EPubReaderChapterList) {
        updateCurrentPage()
