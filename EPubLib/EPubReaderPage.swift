...
@objc public protocol EPubReaderPageDelegate: class {
	...
	/**
	 ...
	 - parameter page: The loaded page
	 */
	@objc optional func pageDidLoad(_ page: EPubReaderPage)
}

open class EPubReaderPage: UICollectionViewCell, UIWebViewDelegate, UIGestureRecognizerDelegate {
    
    weak var delegate: EPubReaderPageDelegate?
	/// The index of the current page. Note: The index start at 1!
	open var pageNumber: Int!
	var webView: EPubReaderWebView!
    ...
    override public init(frame: CGRect) {
        ...
        if webView == nil {
            webView = EPubReaderWebView(frame: webViewFrame())
    ...
    func webViewFrame() -> CGRect {
        ...
        let navBarHeight = EPubReader.shared.readerCenter?.navigationController?.navigationBar.frame.size.height ?? CGFloat(0)
...
//		}
    ...
    open func webViewDidFinishLoad(_ webView: UIWebView) {
		guard let webView = webView as? EPubReaderWebView else {
			return
		}
...
//            
//            if let audioPlayer = EPubReader.shared.readerAudioPlayer , audioPlayer.isPlaying() {
//                audioPlayer.readCurrentSentence()
//            }
//        }
        
        let direction: ScrollDirection = EPubReader.needsRTLChange ? .positive() : .negative()
    ...
    open func webView(_ webView: UIWebView, shouldStartLoadWith request: URLRequest, navigationType: UIWebViewNavigationType) -> Bool {
         guard
            let webView = webView as? EPubReaderWebView,
        ...
        /*else if scheme == "play-audio" {
            ...
            let chapter = EPubReader.shared.readerCenter?.getCurrentChapter()
            let href = chapter?.href ?? ""
            EPubReader.shared.readerAudioPlayer?.playAudio(href, fragmentID: playID)
            ...
            if (url.path as NSString).pathExtension != "" {
                ...
                let hrefPage = (EPubReader.shared.readerCenter?.findPageByHref(href) ?? 0) + 1
                ...
                if hrefPage == pageNumber {
                    ...
                    EPubReader.shared.readerCenter?.changePageWith(href: href, animated: true)
                }
            ...
            if #available(iOS 9.0, *) {
                ...
                EPubReader.shared.readerCenter?.present(safariVC, animated: true, completion: nil)
            } else {
                let webViewController = WebViewController(url: request.url!)
                let nav = UINavigationController(rootViewController: webViewController)
                nav.view.tintColor = readerConfig.tintColor
                EPubReader.shared.readerCenter?.present(nav, animated: true, completion: nil)
            }
            return false
		} else {
    ...
    open func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldRecognizeSimultaneouslyWith otherGestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        if gestureRecognizer.view is EPubReaderWebView {
...
//        webView.setMenuVisible(false)
		...
		if	let _navigationController = EPubReader.shared.readerCenter?.navigationController , _navigationController.isNavigationBarHidden {
            ...
            if selected == nil || selected!.characters.count == 0 {
                ...
                DispatchQueue.main.asyncAfter(deadline: dispatchTime, execute: {
                    ...
                    if self.shouldShowBar && !menuIsVisibleRef {
                        EPubReader.shared.readerCenter?.toggleBars()
                    }
                    self.shouldShowBar = true
                })
            }
        } else if readerConfig.shouldHideNavigationOnTap == true {
            EPubReader.shared.readerCenter?.hideBars()
        }
	...
	*/
    ...
    func audioMarkID(_ ID: String) {
        guard let currentPage = EPubReader.shared.readerCenter?.currentPage else { return }
