//
//  EPubReaderWebView.swift
//  EPubReaderKit
//
//  Created by Hans Seiffert on 21.09.16.
//  Copyright (c) 2016 EPub Reader. All rights reserved.
//

import UIKit

/// The custom WebView used in each page 
open class EPubReaderWebView: UIWebView {
	var isColors = false
	var isShare = false
    var isOneWord = false
	...
	func share(_ sender: UIMenuController) {
		...
		let shareImage = UIAlertAction(title: readerConfig.localizedShareImageQuote, style: .default, handler: { (action) -> Void in
			if self.isShare {
				if let textToShare = self.js("getHighlightContent()") {
					EPubReader.shared.readerCenter?.presentQuoteShare(textToShare)
				}
			} else {
		...
		let shareText = UIAlertAction(title: readerConfig.localizedShareTextQuote, style: .default) { (action) -> Void in
			if self.isShare {
				if let textToShare = self.js("getHighlightContent()") {
					EPubReader.shared.readerCenter?.shareHighlight(textToShare, rect: sender.menuFrame)
				}
			} else {
				if let textToShare = self.js("getSelectedText()") {
					EPubReader.shared.readerCenter?.shareHighlight(textToShare, rect: sender.menuFrame)
        ...
        if let alert = alertController.popoverPresentationController {
            alert.sourceView = EPubReader.shared.readerCenter?.currentPage
            alert.sourceRect = sender.menuFrame
        }
        
		EPubReader.shared.readerCenter?.present(alertController, animated: true, completion: nil)
	}
	...
	func define(_ sender: UIMenuController?) {
		...
		EPubReader.shared.readerContainer.show(vc, sender: nil)
	}

	func play(_ sender: UIMenuController?) {
		//EPubReader.shared.readerAudioPlayer?.play()
